...
> **TimesFM price forecasting**: disabled by default (`timesfm.enabled: false`) due to high RAM usage on CPU (4–6 GB). To enable: set `enabled: true` in `settings.yaml` — requires at least 8 GB free RAM. The subprocess isolation mode is planned for v1.1.
...
> **Authentication**: the Admin panel uses a two-factor authentication system. Credentials are managed through the dashboard's user management interface — no plain-text password in `.env`.
...
└── utils/
    ...
    └── i18n.py                      # Dashboard internationalization
```
...
### v1.1 (next)
- [ ] TimesFM price forecasting (subprocess isolation for CPU offload)
