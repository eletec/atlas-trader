...
from quant.features import compute_features, compute_rally_features, make_target_direction
...
class LiveRunner:
    ...
    def __init__(self, symbol="BTC/USDT", timeframe="5m", history_days=90,
        ...
        self._cooldown_until_ts: float = 0.0   # Fix1+Fix3 : blocage après toute sortie perdante
    ...
    def _refit(self):
        ...
        # ── Rally features V3.1 ──
        rally = compute_rally_features(ohlcv)
        feats_all = pd.concat([feats_all, rally], axis=1)
    ...
    def step(self, trigger="scheduled"):
        ...
        # ── Rally features (V3.1 — consensus 3 IA) ──
        # Computed AFTER feats_all to add multi-TF context features
        rally_feats = compute_rally_features(ohlcv)
        # Drop any overlapping columns from previous cycles
        feats_all = feats_all.drop(columns=[c for c in rally_feats.columns if c in feats_all.columns], errors="ignore")
        feats_all = pd.concat([feats_all, rally_feats], axis=1)
        ...
        # ── Multi-TF context (V3.1) ──
        # Compute trend_4h and trend_1h BEFORE decide() so they can drive the signal
        trend_4h_up: bool | None = None
        trend_1h_up: bool | None = None
        if self._ohlcv_1h is not None and len(self._ohlcv_1h) >= 100:
            try:
                ...
                _ohlcv_4h = c1h.resample("4h").last().dropna()
                if len(_ohlcv_4h) >= 50:
        ...
        # ── Rally features for the current bar ──
        _rally = feats_all["rally_pct"].iloc[-1] if "rally_pct" in feats_all.columns else 0.0
        _rsi   = feats_all["rsi_14"].iloc[-1] if "rsi_14" in feats_all.columns else 50.0
        _atr_pct_val = feats_all["atr_pct"].iloc[-1] if "atr_pct" in feats_all.columns else 0.0
        _regime_name = "PANIC" if not regime_trending and not regime_ranging else \
                       "TREND" if regime_trending else "RANGE"
        ...
        decision = decide(
            ...
            rally_pct=float(_rally) if pd.notna(_rally) else 0.0,
            rsi=float(_rsi) if pd.notna(_rsi) else 50.0,
            atr_pct=float(_atr_pct_val) if pd.notna(_atr_pct_val) else 0.0,
        ...
        if self._position is not None:
            ...
            if exit_signal:
                ...
                # Fix1+Fix3 : cooldown 3h après TOUTE sortie perdante (SL ou time_exit)
                # pour casser la boucle "SHORT → SL par rebond → re-SHORT → re-SL"
                # observé 05-06/06/2026 — marché baissier avec rebonds intra-trend
                if trade_result.get("pnl_abs", 0) < 0:
                    _cooldown_s = _TF_SECONDS.get(self.timeframe, 300) * 36  # 36 barres ≈ 3h
                    self._cooldown_until_ts = time.time() + _cooldown_s
                    logger.info(
                        f"[{self.symbol}] cooldown 3h activé après sortie perdante "
                        f"({trade_result['pnl_abs']:+.2f}$ — {trade_result.get('exit_reason', '?')})"
                    )
        ...
        if (self._position is None
            ...
            try:
                pos = risk_mgr.compute_position(
                    ...
                    atr_1h=atr_1h, regime_name=_regime_name,
                )
