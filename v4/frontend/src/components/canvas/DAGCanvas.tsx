  ...
  useEffect(() => {
    if (hydrated) {
      ...
      const schemaMap: Record<string, {in: string[], out: string[]}> = {
        ...
        PaperTrader: {in: ["decision","symbol"], out: ["trade_result"]},
        RecordDecision: {in: ["decision"], out: []},
        LLMNode: {in: ["decision","funding_rate","annual_funding_pct","signal","size_usd","expected_return"], out: ["response","parsed"]},
      };
